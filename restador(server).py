...
def handle_client(client_socket):
    ...
    while True:
        ...
        # Adquirimos el candado antes de modificar el restador
        lock.acquire()
        if(restador<=0):
            break
        restador -= numero
        print(f"Queda: {restador}")
        
         # Liberamos el candado después de modificar el restador
        lock.release()
        
        # Enviamos al cliente la suma acumulada
        client_socket.send(str(restador).encode("utf-8"))
